...
async function getQuestionsByTopic(topicId, options = {}) {
  const { tableName = DYNAMODB_TABLE_NAME, dynamoDBClient = getDynamoDBClient() } =
    options;
 ...
 */
async function getAllQuestionsByTopic(tableName, topicId, dynamoDBClient = null) {
  ...
  try {
    ...
    logger.error(`Failed to get all questions for topic ${topicId}: ${error.message}`);
